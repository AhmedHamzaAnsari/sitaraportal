...
LEFT JOIN
action_overspeed as ao on ao.alert_id=ov.id WHERE DATE(ov.created_at) = '$from' and ov.cartraige_id='$cart_id' and ov.time_peiod='$interval'");

$excess_driving = mysqli_query($db, "SELECT 
ov.*,IF(ao.id IS NOT NULL AND ao.id IS NOT NULL, 1, 0) AS join_result
FROM activity_excess_driving_voiltion as ov
LEFT JOIN
action_excess_driving as ao on ao.alert_id=ov.id WHERE DATE(ov.created_at) = '$from' and ov.cartraige_id='$cart_id' and ov.time_peiod='$interval'");

$resultss_nr = mysqli_query($db, "SELECT 
ov.*,IF(ao.id IS NOT NULL AND ao.id IS NOT NULL, 1, 0) AS join_result 
FROM activity_nr_voiltion as ov
LEFT JOIN
action_nr as ao on ao.alert_id=ov.id
WHERE DATE(ov.created_at) = '$from' and ov.cartraige_id='$cart_id' and ov.time_peiod='$interval'");

$resultss_night = mysqli_query($db, "SELECT 
ov.*,IF(ao.id IS NOT NULL AND ao.id IS NOT NULL, 1, 0) AS join_result 
FROM activity_night_voilation_voiltion as ov
LEFT JOIN
action_night_voilation as ao on ao.alert_id=ov.id
WHERE DATE(ov.created_at) = '$from' and ov.cartraige_id='$cart_id' and ov.time_peiod='$interval'");
...
LEFT JOIN
action_black_spot as ao on ao.alert_id=ov.id WHERE DATE(ov.created_at) = '$from' and ov.cartraige_id='$cart_id' and ov.time_peiod='$interval'");
