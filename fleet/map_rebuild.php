...
<body style="min-height: 700px;">
    ...
    if (prev == 'Cartraige' || prev == 'End-User' || prev == 'karachi_base') {
    ...
    function change_id() {
        ...
        ///Vehicle Search
        var settings = {
            "url": "http://151.106.17.246:8080/sitara_api/sitara/total.php?accesskey=12345&user=" + user_id + "",
            "method": "GET",
            "timeout": 0,
        };
    ...
    function distributor() {
        ...
        $('#loading').show();
        $.ajax({
            url: "http://151.106.17.246:8080/sitara_api/sitara/distributor.php?accesskey=12345",
            method: "GET",
            timeout: 0,
            cache: false,
    ...
    $('#dis').on('click', ".ss2", function() {
        ...
        $('#loading').show();
        $.ajax({
            url: "http://151.106.17.246:8080/sitara_api/sitara/end_user.php?dis=" + my_id +
                "&accesskey=12345",
            method: "GET",
            timeout: 0,
            cache: false,
    ...
    $('#endu').on('click', ".ss3", function() {
        ...
        ///Vehicle Search
        var settings = {
            "url": "http://151.106.17.246:8080/sitara_api/sitara/total.php?accesskey=12345&user=" +
                user_id + "",
            "method": "GET",
            "timeout": 0,
        };
    ...
    $(document).ready(function() {
        ...
        var refreshId = setInterval(function() {
            if (vehcile_selection.length == 1) {
                ...
                var settings = {
                    "url": "http://151.106.17.246:8080/sitara_api/sitara/selected_vehicle.php?vehi=" +
                        vehcile_selection + "&accesskey=12345",
                    "method": "GET",
                    "timeout": 0,
                };
                //console.log("http://151.106.17.246:8080/sitara_api/sitara/selected_vehicle.php?vehi=" + vehcile_selection + "&accesskey=12345");
        ...
        $("#myInput").keydown(function() {
            ...
            ///Vehicle Search
            var settings = {
                "url": "http://151.106.17.246:8080/sitara_api/sitara/vehi_search.php?accesskey=12345&str=" +
    ...
    function updated_data() {
        ...
        if (focused == 0) {
            ...
            var settings = {
                "url": "http://151.106.17.246:8080/sitara_api/sitara/selected_vehicle.php?vehi=" + focused +
                    "&accesskey=12345",
                "method": "GET",
                "timeout": 0,
            };
            //console.log("http://151.106.17.246:8080/sitara_api/sitara/selected_vehicle.php?vehi=" + focused + "&accesskey=12345");
    ...
    $("#stop").click(function() {
        ...
        ///Vehicle Search
        var settings = {
            "url": "http://151.106.17.246:8080/sitara_api/sitara/stopped_vehicle.php?accesskey=12345&user=" +
                user_id + "",
            "method": "GET",
            "timeout": 0,
        };
    ...
    $("#nodata").click(function() {
        ...
        ///Vehicle Search
        var settings = {
            "url": "http://151.106.17.246:8080/sitara_api/sitara/nodata.php?accesskey=12345&user=" +
                user_id + "",
            "method": "GET",
            "timeout": 0,
        };
    ...
    $("#running").click(function() {
        ...
        ///Vehicle Search
        var settings = {
            "url": "http://151.106.17.246:8080/sitara_api/sitara/running.php?accesskey=12345&user=" +
                user_id + "",
            "method": "GET",
            "timeout": 0,
        };
    ...
    $("#idle").click(function() {
        ...
        ///Vehicle Search
        var settings = {
            "url": "http://151.106.17.246:8080/sitara_api/sitara/idle.php?accesskey=12345&user=" + user_id +
                "",
            "method": "GET",
            "timeout": 0,
        };
    ...
    $("#inactive").click(function() {
        ...
        ///Vehicle Search
        var settings = {
            "url": "http://151.106.17.246:8080/sitara_api/sitara/inactive.php?accesskey=12345&user=" +
                user_id + "",
            "method": "GET",
            "timeout": 0,
        };
    ...
    // $('#d2').on('click', ".vehicle-checkbox", function (e) {
    //     e.stopPropagation(); // Prevent the click event from bubbling up to the parent .ss element
    // });
    //////////////
    $('#d2').on('click', ".vehicle-checkbox", function(e) {
        var aa = $(this);
        e.stopPropagation()
        // var checkbox = $(this).find('.vehicle-checkbox:first');
        // checkbox.prop('checked', !checkbox.prop('checked'));
        if (!aa.hasClass('bg-light')) {
            aa.addClass('bg-light');

            var my_id = this.id;

            if (!image_selected.includes(my_id)) {
                image_selected.push(my_id);

                var settings = {
                    "url": "http://151.106.17.246:8080/sitara_api/sitara/selected_vehicle.php?vehi=" +
                        image_selected.join(',') + "&accesskey=12345",
                    "method": "GET",
                    "timeout": 0,
                };
                ...
                $.ajax(settings).done(function(response) {
                    const data = JSON.parse(response);
                    var l = Math.min(data.length, 100); // Limiting the loop to 100 items or less

                    for (var i = 0; i < l; i++) {
                        if (Object.keys(markers).length > 0 && vehcile_selection.indexOf(data[i][
                                'id'
                            ]) == -1) {
                            // Create marker only if it doesn't already exist and there are markers on the map
                            createMarker(data[i]);
                        } else if (Object.keys(markers).length === 0) {
                            // Create marker if there are no markers on the map
                            createMarker(data[i]);
                        }
                    }
                });
    ...
    function createMarker(data) {
        ...
        document.getElementById("odometer").innerHTML = data['odometer'];
        document.getElementById("locations").innerHTML = data['location'];
        document.getElementById("latlng").innerHTML = lat + "," + lng;

        var color;
        var icon;

        if (ignition == "Off" && speed == 0) {
            color = "red";
            icon = "assets/images/truck2.svg";
        } else if (ignition == 'On' && speed > 0 && speed <= 50) {
            color = "#1D738D";
            icon = "assets/images/truck.svg";
        } else if (ignition == 'On' && speed == 0) {
            color = "yellow";
            icon = "assets/images/truck.svg";
        } else if (ignition == 'On' && speed > 50) {
            color = "#e62e2d";
            icon = "assets/images/truck.svg";
        } else {
            color = "#c34c9c";
        ...
        vehcile_selection.push(data['id']);
    ...
    function activity() {
        var settings = {
            "url": "http://151.106.17.246:8080/sitara_api/sitara/running_count.php?vehi=" + focused +
                "&accesskey=12345",
            "method": "GET",
            "timeout": 0,
        };
    ...
    function speedo(id) {
        var settings = {
            "url": "http://151.106.17.246:8080/sitara_api/sitara/speed.php?vehi=" + id + "&accesskey=12345",
            "method": "GET",
            "timeout": 0,
        };
    ...
    function basedata(id) {
        ...
        var settings = {
            "url": "http://151.106.17.246:8080/sitara_api/sitara/base_data.php?vehi=" + id + "&accesskey=12345",
            "method": "GET",
            "timeout": 0,
        };
    ...
    function drawpath() {
        var settings = {
            "url": "http://151.106.17.246:8080/sitara_api/sitara/route.php?vehi=" + focused + "&accesskey=12345",
            "method": "GET",
            "timeout": 0,
        };
    ...
    function callVehic(pager, offseter) {
        ...
        offset = offseter + 10;
        var settings = {
            "url": "http://151.106.17.246:8080/sitara_api/sitara/sitara_all_vehi.php?accesskey=12345&user=" +
    ...
    function generateCard(data) {
        var div = '';
        var server_time = moment(data['time']).format('MM/DD/YYYY hh:mm A');
        var current_time = moment().format('MM/DD/YYYY hh:mm A');
        var a = moment(server_time, 'MM/DD/YYYY hh:mm A');
        var b = moment(current_time, 'MM/DD/YYYY hh:mm A');
        ...
        var dotColor = '';
        if (data['ignition'] == 'Off' && data['speed'] == 0) {
            dotColor = 'red';
        } else if (data['ignition'] == 'On' && data['speed'] == 0) {
            dotColor = '#f8e53b';
        } else if (data['speed'] > 0 && data['ignition'] == 'On') {
            dotColor = 'green';
        } else if (diffMinutes > 1440) {

            dotColor = 'blue';
        } else if (data['speed'] > 55) {
            dotColor = 'red';
        }

        div += '<li class="list-unstyled" data-geo-lat="' + data['lat'] + '" data-geo-long="' + data['lng'] + '">';
        div += '<div class="card p-0 mb-1 ss" id="">';
        div += '<div class="card-body p-2">';
        div += '<div class="row p-0 mb-0">';
        div += '<div class="col-lg-1"><input type="checkbox" name="vehicle" value="' + data['id'] + '" id="' + data[
            'id'] + '" class="vehicle-checkbox"></div>';
        div += '<div class="col-lg-5">';
        div += '<h4 class="card-title mb-1" style="vertical-align: middle;display: flex;">';
        div += '<span style="background-color: ' + dotColor + '" class="dot"></span>';
        div += '&nbsp;' + data['name'] + '</h4></div>';
        div += '<div class="col-lg-6" style="text-align: right;">';
        div += '<p class="p-0 font-size-12 mb-1" id="' + data['id'] + '-time">' + data['time'] + '</p></div>';
        div += '</div><div class="row p-0 mb-0 mt-0"><div class="col-lg-5">';
        div += '<i class="fas fa-tachometer-alt font-size-10"></i>&nbsp;' + data['speed'] + ' km/h</div>';
        div += '<div class="col-lg-7" style="text-align: right;">';
        div += '<i class="fas fa-key" style="color: rgb(185, 8, 8);"></i>&nbsp;';
        div += '<i class="fas fa-power-off" style="color: rgb(8, 185, 17);"></i></div></div>';
        div += '<div class="row p-0 mb-0 mt-0"><div class="col-lg-7">';
        div += '<i class="fas fa-map-marker-alt font-size-10" style="color: rgb(33, 114, 189);"></i>&nbsp;' + data[
            'location'] + '</div>';
        div += '<div class="col-lg-5" style="text-align: right;">';
        div += '<i class="fas fa-building" style="color: rgb(240, 34, 96);"></i>&nbsp;' + data['trackername'] +
            '</div>';
        div += '</div></div></div></li>';
    ...
    function count() {
        var settings = {
            "url": "http://151.106.17.246:8080/sitara_api/sitara/counts.php?accesskey=12345&user=" + user_id + "",
            "method": "GET",
            "timeout": 0,
        };
