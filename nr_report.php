...
$id = $_SESSION['userid'];
 $nr_sql = "SELECT dc.name,dc.tracker as vehicle_make,dc.time,dc.speed,dc.location as vlocation ,dc.lat as latitude,dc.lng as longitude FROM users_devices_new as ud 
join devicesnew as dc on dc.id=ud.devices_id
 where dc.time <='$prev_date' and ud.users_id=$id group by dc.name order by dc.time desc";
$resultss = mysqli_query($db,$nr_sql);
...
<body class="sidebar-noneoverflow starterkit">
    ...
    function send_mail() {
        ...
        $.ajax({
            url: 'sitara_schedule_email/nr_report.php',
            type: 'POST',
            data: {
                email_arr: email_arr
            },
