...
function fetch_customer_data($connect)
{
   $today = date("Y-m-d");
	...
	// $query = "SELECT dc.name,geo.consignee_name,geo.location,gca.in_time,gca.out_time,gca.in_duration,geo.Coordinates FROM geofenceing as geo join geo_check_audit as gca on gca.geo_id = geo.id join devices as dc on gca.veh_id=dc.uniqueId where geo.geotype='Black Spote' and gca.in_time >='$today';";
	$query = "SELECT dc.name,geo.consignee_name,geo.location,gca.in_time,gca.out_time,gca.in_duration,geo.Coordinates FROM geofenceing as geo join geo_check_audit as gca on gca.geo_id = geo.id join devices as dc on gca.veh_id=dc.uniqueId  join users_devices as docc on docc.devices_id=dc.uniqueId where docc.users_id='$user_id' and geo.geotype='Black Spote' and gca.in_time >='$today';";
    // echo $query;
