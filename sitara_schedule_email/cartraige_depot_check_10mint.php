...
<?php
            ...
            $sql="SELECT cd.cartrauge_id,us.name FROM cartrauge_depot_check as cd join users as us on us.id=cd.cartrauge_id where cd.sms_alerts='black spot';";
                ...
                while( $row = mysqli_fetch_array($result) ){
                    ...
                    if($count_devices > 0) {
                        while( $row = mysqli_fetch_array($result_devices) ){ 
                            ...
                            where geo.geotype='black Spote' and gc.in_time>'$date 00:00:00' and ud.users_id='$cartraige_id' and ud.devices_id='$devices_id' and gc.log=0 and gc.depot_status='1' group by veh_id order by gc.id desc";
                            ...
                            if($count_depot_status > 0) {
                                while( $row = mysqli_fetch_array($result_depot_status) ){ 
                                    ...
                                    $veh_id = $row['veh_id'];
                                    $geo_check_id = $row['id'];
                                    $check_geo_log = $row['log'];
                                    $vehicle_name = $row["name"];
                                    $Coordinates = $row["Coordinates"];


                                    echo '-------------------------------- <br>';
                                    echo $in_time .'<br> '.$geo_id.' <br>'.$veh_id.' <br>';
                                    ...
                                    if($diffInMinutes>10){
                                        ...
                                        if($check_geo_log=='0'){
                                            ...
                                            if (mysqli_query($db, $sql_update_depot_status)) {
                                                ...
                                                if ($count_contact > 0) {
                                                    while ($row = mysqli_fetch_array($result_contact)) {
                                                        ...
                                                        CURLOPT_URL => 'https://connect.jazzcmt.com/sendsms_url.html?Username=03028652867&Password=Jazz@123&From=Go Get Going With Go -LIVE&To='.$contact.'&Message='.urlencode($msg),
                                            ...
                                            $sql_update_depot_status = "UPDATE geo_check set depot_status='2' where veh_id='$veh_id' and geo_id='$geo_id' and id='$geo_check_id'";
                                            if (mysqli_query($db, $sql_update_depot_status))
                                            {
                                                echo "Log update successfully ! <br>";
