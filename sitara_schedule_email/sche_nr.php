...
function fetch_customer_data($connect)
{
	...
	$query = "SELECT dv.name,pos.time,pos.speed,pos.vlocation FROM `positions` as pos INNER join `devices` as dv on pos.id=dv.latestPosition_id INNER JOIN `users_devices` as ud on dv.uniqueId=ud.devices_id where pos.time <='$prev_date' and ud.users_id='$user_id' group by dv.name order by pos.time desc";
