...
">
    ...
    $vehicle=mysqli_query($db, "SELECT count(*) as t_trip FROM trip_main as tm  
    join trip_sub as ts on ts.main_id=tm.id  
    join users_devices as ud on ud.devices_id=ts.vehicle_id 
    join users as ur on ud.users_id=ur.id 
    where ud.users_id='$user_id' and tm.user_id!='';");
    $countdata=mysqli_fetch_array($vehicle);

    $vehicle_movw=mysqli_query($db, "SELECT count(*) as on_trip FROM trip_main as tm  
    join trip_sub as ts on ts.main_id=tm.id  
    join users_devices as ud on ud.devices_id=ts.vehicle_id 
    join users as ur on ud.users_id=ur.id 
    where ud.users_id='$user_id' and tm.user_id!='' and ts.status='0';");
    $count_move=mysqli_fetch_array($vehicle_movw);

    $vehicle_idel=mysqli_query($db, "SELECT count(*) as comp_trip FROM trip_main as tm  
    join trip_sub as ts on ts.main_id=tm.id  
    join users_devices as ud on ud.devices_id=ts.vehicle_id 
    join users as ur on ud.users_id=ur.id 
    where ud.users_id='$user_id' and tm.user_id!='' and ts.status='1';");
    $count_idel=mysqli_fetch_array($vehicle_idel);

    $vehicle_stop=mysqli_query($db, "SELECT count(*) as forced_trip FROM trip_main as tm  
    join trip_sub as ts on ts.main_id=tm.id  
    join users_devices as ud on ud.devices_id=ts.vehicle_id 
    join users as ur on ud.users_id=ur.id 
    where ud.users_id='$user_id' and tm.user_id!='' and ts.status='2';");
    $count_stop=mysqli_fetch_array($vehicle_stop);

    $vehicle_speed=mysqli_query($db, "SELECT count(*) as r_trip FROM trip_main as tm  
    join trip_sub as ts on ts.main_id=tm.id  
    join users_devices as ud on ud.devices_id=ts.vehicle_id 
    join users as ur on ud.users_id=ur.id 
    where ud.users_id='$user_id' and tm.user_id!='' and ts.id  in (SELECT trip_sub_id FROM receive_sub);");
    $count_speed=mysqli_fetch_array($vehicle_speed);

    $not_reciving=mysqli_query($db, "SELECT count(*) as not_r_trip FROM trip_main as tm  
    join trip_sub as ts on ts.main_id=tm.id  
    join users_devices as ud on ud.devices_id=ts.vehicle_id 
    join users as ur on ud.users_id=ur.id 
    where ud.users_id='$user_id' and tm.user_id!='' and ts.id not in (SELECT trip_sub_id FROM receive_sub);");
    ...
    $result12=mysqli_query($db, "SELECT ts.*,tm.driver_name,tm.location,tm.without_tracker,geo.consignee_name as base,toc.close_time FROM trip_main as tm join trip_sub as ts on ts.main_id=tm.id join users_devices as ud on ud.devices_id=ts.vehicle_id join users as ur on ud.users_id=ur.id join geofenceing as geo on geo.id=tm.base left join trip_close as toc on toc.sub_id=ts.id where ud.users_id='$user_id' and tm.user_id!='';");
    // echo "SELECT * FROM trip_main as tm join trip_sub as ts on ts.main_id=tm.id join users_devices as ud on ud.devices_id=ts.vehicle_id join users as ur on ud.users_id=ur.id where ud.users_id='$user_id' and tm.user_id!='';";
                                        ...
                                        <div class="col-md-3">
                                            <div class="card border-primary mx-sm-1 p-3">
                                                ...
                                                <div class="text-primary text-center mt-2">
                                                    <h1 ><?php echo $count_not_reciving['not_r_trip'];?></h1>
                                                </div>
                                            </div>
                                        </div>
