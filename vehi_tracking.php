...
<head>
    ...
    <title>Go Get Going With Go  Vehicle Tracking</title>
...
if($data['privilege']==='Admin' || $data['privilege'] === 'Depot' || $data['privilege'] === 'viewer'){

    $users = mysqli_query($db,"SELECT * FROM `users` WHERE   id='$idd'");
}

$admin_vehi = mysqli_query($db,"SELECT count(*) as admin_vehi FROM users_devices_new as ud join devicesnew as dc on dc.id=ud.devices_id where ud.users_id=1");
                            ...
                            <div class="col-lg-12">
                                ...
                                <div class="container" style="height: 90vh;overflow: auto;">
                                    ...
                                    <ul id="my-tree" class="file-tree">
                                        ...
                                        <?php if($data['privilege']==='Admin' || $data['privilege']==='viewer' || $data['privilege']==='Depot' ){?>
                                        <?php foreach($users as $key => $value1){ 
                                            if($data['privilege']==='Depot' || $data['privilege']==='viewer'){
                                            ...
                                            $b = mt_rand(100000,9999969); ?>
                                        <li class="file-tree-folder">
                                            <input type="checkbox" name="myCheck" id="userCheck<?= $b ?>"
                                                value="<?= $cid; ?>" onclick="users_car('<?= $cid; ?>','<?= $b; ?>')">
                                            <span style="font-weight:bold"><?= $value1['name']; ?></span><br>
                                            ...
                                            <ul>
                                                <?php 
                                                        ...
                                                        foreach($distribut as $key => $value2){ 
                                                            if($data['privilege']==='Depot'){

                                                                $cidd=2; 
                                                            }
                                                            else{
                                                                $cidd=$value2['id'];
                                                                // echo '<script>alert('.$cidd.')</script>';

                
                                                            }
                                                            // $cidd=$value2['id']; 
                                                            $c = mt_rand(100000,9995999); $did=$value2['id']; ?>

                                                <li class="file-tree-folder"><input type="checkbox" name="myCheck"
                                                        id="userCheck<?= $c ?>" value="<?= $cidd; ?>"
                                                        onclick="users_car('<?= $cidd; ?>','<?= $c; ?>')">
                                                    <?= $value2['name']; ?><p>Total Vehicle : (<span
                                                            id="dis__vehi<?= $cidd; ?>"></span>)</p>
                                                    ...
                                                    <ul>
                                                        ...
                                                        <li class="file-tree-folder"> <input type="checkbox"
                                                            ...
                                                            <ul>
                                                                <li>
                                                                    <?php
                                                                            $query4="SELECT ud.users_id,ud.devices_id,us.name,dc.name,UPPER(dc.name) as car_name,dc.ignition as power,dc.lat as latitude,dc.lng as longitude,dc.id as device_id,dc.speed,dc.odometer as imileage,dc.time FROM users_devices_new as ud  INNER JOIN users as us on ud.users_id=us.id INNER JOIN devicesnew as dc on ud.devices_id=dc.id  where ud.show_authority=1 and ud.users_id=$eid";
                                                                            // echo $query;
                                                                            $results4 = mysqli_query($db, $query4);

                                                                            $end_vehi = mysqli_query($db,"SELECT count(*) as end_vehi FROM `users_devices_new` where users_id=$eid");
                                                                            // echo "SELECT count(*) as end_vehi FROM `users_devices_new` where users_id=$eid";
                                                ...
                                                <?php
                                                                            $query1="SELECT ud.users_id,ud.devices_id,us.name,dc.name ,UPPER(dc.name) as car_name,pos.latitude,pos.longitude,pos.device_id,pos.speed,pos.imileage,pos.time FROM users_devices_new as ud INNER JOIN users as us on ud.users_id=us.id INNER JOIN devicesnew as dc on ud.devices_id=dc.id INNER JOIN positions as pos on dc.latestPosition_id=pos.id where ud.show_authority=1 and ud.users_id=$cidd";
                                                                            // echo $query;
                                                                            $results1 = mysqli_query($db, $query1);
                                                                            // echo "SELECT count(*) as end_vehi FROM `users_devices_new` where users_id=$cidd";
                                                                            $dis_vehi = mysqli_query($db,"SELECT count(*) as dis_vehi FROM `users_devices_new` where users_id=$cidd");
                                        ...
                                        <?php } ?>
                                        <?php
                                                        $query_a="SELECT ud.users_id,ud.devices_id,us.name,dc.name,UPPER(dc.name) as car_name,dc.ignition as power,dc.lat as latitude,dc.lng as longitude,dc.id as device_id,dc.speed,dc.odometer as imileage,dc.time FROM users_devices_new as ud  INNER JOIN users as us on ud.users_id=us.id INNER JOIN devicesnew as dc on ud.devices_id=dc.id where ud.show_authority=0 and ud.users_id=$cid";
                                                        // echo $query;
                                                        $results_d = mysqli_query($db, $query_a);

                                                        

                                                        $i=1;
                                                        foreach ($results_d as $city){
                                                            $a = $city['devices_id'];
                                                ...
                                                <ul>
                                                    <li>
                                                        <?php
                                                                        ...
                                                                        $end_vehi = mysqli_query($db,"SELECT count(*) as end_vehi FROM `users_devices_new` as ud join devicesnew as dc on dc.id=ud.devices_id where users_id=$eid");
                                                                        // echo "SELECT count(*) as end_vehi FROM `users_devices_new` where users_id=$eid";
                                                                        $countend_vehi=mysqli_fetch_array($end_vehi);
                                                                        $ending=$countend_vehi['end_vehi'];
                                                                        echo "
                                                    ...
                                                    <li>
                                                        ...
                                                        <input type="checkbox" name="myCheck" id="myCheck<?= $e ?>"
                                                            value="<?= $city4['devices_id']; ?>"
                                                            onclick="myFunction('<?= $city4['car_name']; ?>','<?= $e; ?>')">
                                                        <span><img src="images/icon/car_icon_blue.png" alt="" srcset="">
                                                        </span><?= $city4['car_name']; ?>
                                                        <span class=" mt-2"
                                                            style="float:right ;font-size: 10px;"><?= $city4['speed']; ?>
                                                            Stop</span>
                                    ...
                                    <?php
                                                                        ...
                                                                        $results1 = mysqli_query($db, $query1);
                                                                        // echo "SELECT count(*) as end_vehi FROM `users_devices_new` where users_id=$cidd";
                                                                        $dis_vehi = mysqli_query($db,"SELECT count(*) as dis_vehi FROM `users_devices_new` as ud join devicesnew as dc on dc.id=ud.devices_id where users_id=$cidd");
                                                                        $countdis_vehi=mysqli_fetch_array($dis_vehi);
                                                                        $dissing=$countdis_vehi['dis_vehi'];
                                                                        echo "
                                                                            <script type=\"text/javascript\">
                                        ...
                                        <ul>
                                            ...
                                            <li>
                                                ...
                                                <input type="checkbox" name="myCheck" id="myCheck<?= $e ?>"
                                                    value="<?= $city4['devices_id']; ?>"
                                                    onclick="myFunction('<?= $city4['car_name']; ?>','<?= $e; ?>')">
                                                <span><img src="images/icon/car_icon_green.png" alt="" srcset="">
                                                </span><?= $city4['car_name']; ?> <span class=" mt-2"
                                                    style="float:right ;font-size: 10px;"><?= $city4['speed']; ?>
                                                    km/h Moving</span>
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (var i = 0; i < len; i++) {
                        var str = data.car_name;
                        // console.log(data[i])
                        setMarker_users(data[i].latitude, data[i].longitude, data[i].speed, data[i].device_id, data[i].car_name, data[i].vlocation, data[i].time)
                    }
